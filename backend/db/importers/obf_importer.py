"""
Open Beauty Facts CSV dump importer.

Streams the gzipped CSV line-by-line (no full-file load) and batch-upserts
cosmetic products + raw INCI ingredient entries into PostgreSQL.

Usage:
    # Stream from a local file:
    python -m db.importers.obf_importer --file /path/to/en.openbeautyfacts.org.products.csv.gz

    # Stream directly from the OBF CDN:
    python -m db.importers.obf_importer --url

    # Limit rows and skip ingredient linking:
    python -m db.importers.obf_importer --url --limit 50000 --skip-ingredients
"""
...
OBF_DUMP_URL = "https://static.openbeautyfacts.org/data/en.openbeautyfacts.org.products.csv.gz"
...
VALUES ($1, $2, $3, 'cosmetic', $4, $5, $6, $7, 'obf', NOW())
...
def _parse_nova(raw: str) -> Optional[int]:
    ...
    v = raw.strip()
    return int(v) if _NOVA_RE.match(v) else None
...
def _parse_inci(raw: str) -> list[str]:
    """
    Parse an INCI ingredient list.

    INCI lists are comma- and semicolon-separated. Each token is returned
    stripped; empty tokens are discarded.
    """
    if not raw.strip():
        return []
...
def _map_row(row: dict) -> Optional[dict]:
    ...
    # Cosmetics don't have Nutri-Score or NOVA; keep columns NULL
    ...
    # OBF uses ingredients_text / ingredients_text_en as the INCI ingredient list
    ingredients_text = (
...
def _open_gz_stream(path: Optional[str], use_url: bool):
    ...
    Streams from URL or opens a local file.
    """
    if use_url:
        print(f"Streaming from {OBF_DUMP_URL} ...")
        resp = urlopen(OBF_DUMP_URL)
...
async def _flush_batch(
    ...
    Returns (products_upserted, ingredient_rows_inserted).
    """
    products_upserted = 0
    ingredients_rows = 0
    ...
    async with conn.transaction():
        for product in batch:
            ...
            if not skip_ingredients and product["ingredients_text"]:
                # Treat as INCI list
                tokens = _parse_inci(product["ingredients_text"])
...
async def _run_import(
    ...
    try:
        ...
        for raw_row in reader:
            ...
            if rows_seen % LOG_INTERVAL == 0:
                ...
                print(
                    ...
                    f"{total_ingredients:,} INCI rows | {rate:.0f} rows/s]"
                )
                await conn.execute(
                    _UPDATE_SYNC_LOG_PROGRESS,
                    sync_log_id,
        ...
        print(
            ...
            f"  INCI rows saved: {total_ingredients:,}\n"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import Open Beauty Facts CSV dump into the safescan database."
    )
    ...
    source.add_argument(
        "--url",
        action="store_true",
        help=f"Stream directly from {OBF_DUMP_URL}",
    )
    parser.add_argument(
        "--limit",
        type=int,
